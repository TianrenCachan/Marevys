# MARÉVYS RC21 — 部署与真实服务验收

本文件描述 RC21 的塔罗／星盘主流程，替代旧版的固定七日塔罗和本地自动回退说明。代码可以交付，不等于生产站点已升级。

## 本次状态（2026-09-21）

- 源码包含新牌阵、78 张牌、资料摘要检索、单张补牌、本命星盘及无付费会员的浏览器手记。
- 后端专项测试已通过 43 项，供应商响应为模拟值；真实 OpenAI／Redis 服务尚未在本次环境验收。
...
| POST /api/astrology | 确定性本命盘及基础象征解读 | 15 秒 |
| GET /api/locations | 有限出生城市目录 | 10 秒 |

函数 includeFiles 包含所需 server 文件；npm 运行时依赖由部署系统安装。dist 只包含公开网页资产，构建使用白名单暴露牌名和原创摘要。原始 PDF、书籍全文、私有工作目录和填好凭据的环境文件不能进入公开资源或提交。

静态 HTML、CSS、图片发布成功只能证明静态站点可访问，不能证明 Node.js API、Redis 或 OpenAI 已启用。旧离线完整嵌入 HTML 不支持 RC21 全部服务器功能。
